...
class TestRickFullAuto:
    ...
    async def test_full_auto_web_app(self):
        from rick_mcp.models import FullAutoInput
        from rick_mcp.tools.jarvis import rick_full_auto
...
class TestRickKillChain:
    ...
    async def test_status_creates_new(self, tmp_path):
        from rick_mcp.models import KillChainInput
    ...
    @pytest.mark.asyncio
    async def test_add_finding_with_image(self, tmp_path):
        from rick_mcp.models import KillChainInput
        from rick_mcp.tools.jarvis import rick_kill_chain
        ...
        with patch(_PATCH_TARGET, tmp_path):
            await rick_kill_chain(KillChainInput(action="status", engagement_id="img-eng"))
            await rick_kill_chain(KillChainInput(action="advance", engagement_id="img-eng", phase=1))
            result = await rick_kill_chain(
                KillChainInput(
                    action="add_finding",
                    engagement_id="img-eng",
    ...
    @pytest.mark.asyncio
    async def test_add_finding_bad_image(self, tmp_path):
        from rick_mcp.models import KillChainInput
        from rick_mcp.tools.jarvis import rick_kill_chain

        with patch(_PATCH_TARGET, tmp_path):
            await rick_kill_chain(KillChainInput(action="status", engagement_id="badimg"))
            await rick_kill_chain(KillChainInput(action="advance", engagement_id="badimg", phase=1))
            result = await rick_kill_chain(
                KillChainInput(
                    action="add_finding",
                    engagement_id="badimg",
                    finding="test",
...
class TestRickNextMove:
    ...
    async def test_next_move_no_engagement(self, tmp_path):
        from rick_mcp.models import NextMoveInput
        from rick_mcp.tools.jarvis import rick_next_move
    ...
    async def test_next_move_new_engagement(self, tmp_path):
        from rick_mcp.models import KillChainInput, NextMoveInput
        from rick_mcp.tools.jarvis import rick_kill_chain, rick_next_move
...
class TestRickSitrep:
    ...
    async def test_sitrep_new_engagement(self, tmp_path):
        from rick_mcp.models import KillChainInput, SitrepInput
        from rick_mcp.tools.jarvis import rick_kill_chain, rick_sitrep
...
class TestDickHelpers:
    def test_phase_advice_all_phases(self):
        from rick_mcp.tools.jarvis_state import _phase_advice
    ...
    def test_state_file_sanitizes(self):
        from rick_mcp.tools.jarvis_state import _state_file
...
class TestStateHelpers:
    def test_add_mission_log(self, tmp_path):
        from rick_mcp.tools.jarvis_state import KILL_CHAIN_PHASES, _add_mission_log, _load_state, _save_state
    ...
    def test_add_tool_history(self, tmp_path):
        from rick_mcp.tools.jarvis_state import KILL_CHAIN_PHASES, _add_tool_history, _load_state, _save_state
    ...
    def test_add_note(self, tmp_path):
        from rick_mcp.tools.jarvis_state import KILL_CHAIN_PHASES, _add_note, _load_state, _save_state
    ...
    def test_helpers_no_state(self, tmp_path):
        ...
        from rick_mcp.tools.jarvis_state import _add_mission_log, _add_note, _add_tool_history
    ...
    def test_validate_image_path(self, tmp_path):
        from rick_mcp.tools.jarvis_state import _validate_image_path

        img = tmp_path / "test.png"
        img.write_bytes(b"fake")
        result = _validate_image_path(str(img))
        assert result == str(img)

    def test_validate_image_path_none(self):
        from rick_mcp.tools.jarvis_state import _validate_image_path

        assert _validate_image_path(None) is None

    def test_validate_image_path_bad_ext(self, tmp_path):
        from rick_mcp.tools.jarvis_state import _validate_image_path

        bad = tmp_path / "test.exe"
        bad.write_bytes(b"fake")
        with pytest.raises(ValueError, match="Unsupported"):
            _validate_image_path(str(bad))

    def test_validate_image_path_missing(self):
        from rick_mcp.tools.jarvis_state import _validate_image_path

        with pytest.raises(ValueError, match="not found"):
            _validate_image_path("/nonexistent/file.png")

    def test_snapshot_state(self, tmp_path):
        from rick_mcp.tools.jarvis_state import _load_state, _save_state

        with patch(_PATCH_TARGET, tmp_path):
            _save_state("snap-eng", {"id": "snap-eng", "target": "v1"})
            _save_state("snap-eng", {"id": "snap-eng", "target": "v2"}, snapshot=True)
            state = _load_state("snap-eng")
        assert state["target"] == "v2"
        assert len(state["snapshots"]) == 1
        assert state["snapshots"][0]["state"]["target"] == "v1"

    def test_checklist_template(self):
        from rick_mcp.tools.jarvis_state import _get_checklist_template

        items = _get_checklist_template("web_app", 1)
        assert len(items) > 5  # common + web_app extras
        assert all(not item["checked"] for item in items)
        assert all(item["phase"] == 1 for item in items)

    def test_validate_severity(self):
        from rick_mcp.tools.jarvis_state import validate_severity

        assert validate_severity("HIGH") == "high"
        assert validate_severity(None) is None
        with pytest.raises(ValueError):
            validate_severity("banana")

    def test_validate_mitre_id(self):
        from rick_mcp.tools.jarvis_state import validate_mitre_id

        assert validate_mitre_id("T1059") == "T1059"
        assert validate_mitre_id("t1059.001") == "T1059.001"
        assert validate_mitre_id(None) is None
        with pytest.raises(ValueError):
            validate_mitre_id("TXYZ")
